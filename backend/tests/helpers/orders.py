...
async def insert_order(conn: Any, row: dict) -> None:
    ...
    """,
        row["id"], row["tenant_id"], row["pos_event_inbox_id"],
        row["clover_order_id"], row["total_amount_cents"],
        row["state"], row["payment_state"], row.get("payment_state_previous"),
        row.get("channel_label", "unknown"), row.get("channel_category", "unknown"),
        row.get("opened_at"), row.get("closed_at"), row["processed_at"],
...
async def insert_sli(conn: Any, row: dict) -> None:
    ...
    """,
        row["id"], row["tenant_id"], row["order_id"],
        row["clover_line_item_id"], row.get("menu_item_id"),
        row["name_at_sale"], row["quantity"],
        row["price_cents_at_sale"], row.get("discount_amount_cents", 0),
        row["net_revenue_cents"],
        row.get("is_refunded", False), row.get("refunded_quantity", 0),
        row.get("refunded_amount_cents", 0), row.get("is_voided", False),
    )
...
async def seed_inbox_and_order(admin_conn: Any, tenant_id: str) -> dict:
    ...
    """, order_id, tenant_id, inbox_id, clover_id)
