...
_RECIPES_TS = _ROOT / "frontend" / "src" / "api" / "recipes.ts"
...
def _client_paths() -> set[str]:
    """Path templates the TS client calls, normalised to FastAPI route templates."""
    src = _RECIPES_TS.read_text(encoding="utf-8")
    # req<...>(token, `...`) or req<...>(token, '...')  — capture the path (first arg after token)
    raw = re.findall(r"req<[^>]*>\(\s*token,\s*[`'\"]([^`'\"]+)[`'\"]", src)
    out: set[str] = set()
    for p in raw:
        p = p.replace("${menuItemId}", "{menu_item_id}").replace("${modifierId}", "{modifier_id}")
        out.add(f"/api/v1{p}")
    return out
...
def test_client_paths_resolve_to_server_routes() -> None:
    ...
    assert client, "no client paths extracted from recipes.ts — the regex drifted"
