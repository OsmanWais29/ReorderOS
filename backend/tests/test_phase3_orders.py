...
async def test_order_creation_with_set_local(admin_conn, service_conn):
    ...
    stored = await admin_conn.fetchrow(
        "SELECT state, payment_state, payment_state_previous "
...
async def test_service_worker_blocked_without_set_local(admin_conn, service_conn):
    ...
    assert "row-level security" in error_msg or "permission" in error_msg or \
           "policy" in error_msg
...
async def test_menu_item_id_nullable(admin_conn, service_conn):
    ...
    stored = await admin_conn.fetchval(
        "SELECT menu_item_id FROM sale_line_items WHERE id = $1", sli["id"],
    )
...
async def test_check_clock_order(admin_conn, service_conn):
    ...
    with pytest.raises(Exception) as exc:
        async with service_conn.transaction():
            ...
            await service_conn.execute(
                "UPDATE orders SET closed_at = $1, processed_at = $2 "
                "WHERE id = $3",
...
async def test_rls_app_user_tenant_isolation(admin_conn, app_conn, service_conn):
    ...
    async with service_conn.transaction():
        await service_conn.execute(
            f"SET LOCAL app.tenant_id = '{seed_a['tenant_id']}'"
        )
        await insert_order(service_conn, order_a)
    async with service_conn.transaction():
        await service_conn.execute(
            f"SET LOCAL app.tenant_id = '{seed_b['tenant_id']}'"
        )
...
async def test_active_sales_excludes_refunded_voided(admin_conn, service_conn):
    ...
    sli_refunded = make_sli_row({
        ...
        "is_refunded": True, "is_voided": False,
    })
    sli_voided = make_sli_row({
        "tenant_id": seed["tenant_id"],
        "order_id": order["id"],
        "name_at_sale": "Voided Item",
        "is_refunded": False, "is_voided": True,
    })
    ...
    active = await admin_conn.fetch(
        ...
        "WHERE tenant_id = $1 AND NOT is_refunded AND NOT is_voided",
        seed["tenant_id"],
    )
...
async def test_uuidv7_pks(admin_conn, service_conn):
    ...
    stored_order = await admin_conn.fetchval(
        "SELECT id::text FROM orders WHERE id = $1::uuid", order["id"],
    )
    stored_sli = await admin_conn.fetchval(
        "SELECT id::text FROM sale_line_items WHERE id = $1::uuid", sli["id"],
    )
...
async def test_full_order_lifecycle(admin_conn, app_conn, service_conn):
    ...
    sli_active = make_sli_row({
        ...
        "name_at_sale": "Shawarma", "quantity": 2,
        "price_cents_at_sale": 1500, "net_revenue_cents": 3000,
    })
    sli_refunded = make_sli_row({
        "tenant_id": tid, "order_id": order["id"],
        "name_at_sale": "Falafel", "quantity": 2,
        "price_cents_at_sale": 1000, "net_revenue_cents": 2000,
        "is_refunded": True, "refunded_quantity": 1, "refunded_amount_cents": 1000,
    })
    async with service_conn.transaction():
        await service_conn.execute(f"SET LOCAL app.tenant_id = '{tid}'")
    ...
    async with service_conn.transaction():
        ...
        await service_conn.execute("""
            ...
            ON CONFLICT ON CONSTRAINT uq_orders_clover DO NOTHING
        """, str(uuid7()), tid, seed["inbox_id"], clover_oid)
    ...
    async with service_conn.transaction():
        ...
        await service_conn.execute("""
            ...
            ON CONFLICT ON CONSTRAINT uq_sli_clover DO NOTHING
        """, str(uuid7()), tid, order["id"], sli_active["clover_line_item_id"])
    assert 1 == await admin_conn.fetchval(
    ...
    ps = await admin_conn.fetchval(
        "SELECT payment_state FROM orders WHERE id = $1", order["id"],
    )
    assert ps == "REFUNDED"
...
async def test_service_worker_cannot_update_line_items(admin_conn, service_conn):
    ...
    with pytest.raises(Exception) as exc:
        async with service_conn.transaction():
            ...
            await service_conn.execute(
                "UPDATE sale_line_items SET name_at_sale = 'Tampered' "
                "WHERE id = $1", sli["id"],
            )
    assert "permission denied" in str(exc.value).lower() or \
           "insufficient" in str(exc.value).lower()
...
async def test_app_user_cannot_delete(admin_conn, app_conn, service_conn):
    ...
    assert "permission denied" in str(exc_o.value).lower() or \
           "insufficient" in str(exc_o.value).lower()
    ...
    with pytest.raises(Exception) as exc_s:
        async with app_conn.transaction():
            ...
            await app_conn.execute(
                "DELETE FROM sale_line_items WHERE id = $1", sli["id"],
            )
    assert "permission denied" in str(exc_s.value).lower() or \
           "insufficient" in str(exc_s.value).lower()
...
async def test_menu_item_fk_works(admin_conn, service_conn):
    ...
    await admin_conn.execute(
        ...
        menu_item_id, seed["tenant_id"], "Latte",
    )
    ...
    assert "menu_items" in str(exc.value).lower() or \
           "foreign key" in str(exc.value).lower()
